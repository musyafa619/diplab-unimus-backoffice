...
import { createItem, updateItem } from 'src/api/services/items';
...
export function FormDialog({ open, onClose, onFinish, initialData = null }: Props) {
  ...
  const onSubmit = async (values: { name: string; stock: string; description: string }) => {
    setLoading(true);
    try {
      ...
      if (initialData?.id) {
        await updateItem(initialData.id, fd);
      } else {
        await createItem(fd);
      }
  ...
  useEffect(() => {
    if (initialData) {
      ...
      reset({ name: '', description: '', stock: '', imageUrl: '' });
      setPreview(null);
      setFile(null);
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ width: '100%' }}>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              helperText={formState.errors.stock?.message?.toString()}
            />
            <TextField
